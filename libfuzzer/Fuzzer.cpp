...
void Fuzzer::updateTime()
{
  ...
  for(int i=0;i<vulnerabilities.size();i++)
  {
    if(vulnerabilities[i] && recordTime[i] < 0)
    {
      ...
      mutationCount = fuzzStat.totalExecs;
    }
  }
...
*/
  ...
  //Logger::res(to_string(mutationCount));
    ...
    for(int i=0; i<vulnerabilities.size();i++)
    {
      if(vulnerabilities[i])
      {
        if(i==0)
	{
	  Logger::res("\n*****Vulnerability Types: GASLESS_SEND");
	} 
	else if(i==1)
	{
	  Logger::res("\n*****Vulnerability Types: EXCEPTION_DISORDER");
	}
	else if(i==2)
	{
	  Logger::res("\n*****Vulnerability Types: TIME_DEPENDENCY");
	}
	else if(i==3)
	{
	  Logger::res("\n*****Vulnerability Types: NUMBER_DEPENDENCY");
	}
	else if(i==4)
	{
	  Logger::res("\n*****Vulnerability Types: DELEGATE_CALL");
	}
	else if(i==5)
	{
	  Logger::res("\n*****Vulnerability Types: REENTRANCY");
	}
        else if(i==6)
	{
	  Logger::res("\n*****Vulnerability Types: FREEZING");
	}
	else if(i==7)
	{
	  Logger::res("\n*****Vulnerability Types: OVERFLOW");
	}
	else if(i==8)
	{
	  Logger::res("\n*****Vulnerability Types: UNDERFLOW");
	}
      }
